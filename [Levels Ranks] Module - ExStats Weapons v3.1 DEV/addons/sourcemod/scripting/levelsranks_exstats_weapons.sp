...
bool				g_bWeaponsStatusNew[MAXPLAYERS+1][96],
...
void TOPGeneralCount(int iClient)
{
	if(LR_GetClientStatus(iClient))
	{
		...
		g_hDatabase.Query(SQL_TOPGeneralCount, sQuery, iUserID);
	}
}
...
void WeaponsPlayerStats(int iClient)
{
	if(LR_GetClientStatus(iClient))
	{
		...
		g_hDatabase.Query(SQL_WeaponsPlayerStats, sQuery, iUserID);
	}
}
...
{
	char sQuery[196];
	FormatEx(sQuery, sizeof(sQuery), "DELETE FROM `%s_weapons`;", g_sTableName);
	hQuery.AddQuery(sQuery);

	FormatEx(sQuery, sizeof(sQuery), LR_GetDatabaseType() ? "VACUUM `%s_weapons`" : "OPTIMIZE TABLE `%s_weapons`", g_sTableName);
	hQuery.AddQuery(sQuery);
}
...
void LoadDataPlayer(int iClient, int iAccountID)
{
	...
	g_hDatabase.Execute(hTransaction, SQLTransaction_Successful, SQLTransaction_Failure, iUserID, DBPrio_High);
}
...
public void SQLTransaction_Successful(Database db, int iUserID, int numQueries, DBResultSet[] results, any[] queryData)
{
	...
	if(iClient)
	{
		for(int i; i != g_iCountWeapons; i++)
		{
			if(results[i].RowCount && results[i].FetchRow())
			{
				g_iWeaponsStats[iClient][i] = results[i].FetchInt(0);
				g_bWeaponsStatusNew[iClient][i] = false;
			}
			else g_bWeaponsStatusNew[iClient][i] = true;
		}
	}
}
...
void SaveDataPlayer(int iClient, Transaction hQuery)
{
	...
	for(int i; i != g_iCountWeapons; i++)
	{
		...
		{
			if(g_bWeaponsStatusNew[iClient][i])
			{
				FormatEx(sQuery, sizeof(sQuery), "INSERT INTO `%s_weapons` (`steam`, `classname`, `kills`) VALUES ('STEAM_%i:%i:%i', '%s', %i);", g_sTableName, g_iEngine == Engine_CSGO, g_iAccountID[iClient] & 1, g_iAccountID[iClient] >> 1, g_sWeaponsClassName[i], g_iWeaponsStats[iClient][i]);
				g_bWeaponsStatusNew[iClient][i] = false;
			}
			else
			{
				FormatEx(sQuery, sizeof(sQuery), "UPDATE `%s_weapons` SET `kills` = %d WHERE `classname` = '%s' AND `steam` = 'STEAM_%i:%i:%i';", g_sTableName, g_iWeaponsStats[iClient][i], g_sWeaponsClassName[i], g_iEngine == Engine_CSGO, g_iAccountID[iClient] & 1, g_iAccountID[iClient] >> 1);
			}
			hQuery.AddQuery(sQuery);
		}
	}
...
void ResetDataPlayer(int iClient, int iAccountID)
{
	...
	{
		FormatEx(sQuery, sizeof(sQuery), "UPDATE `%s_weapons` SET `kills` = 0 WHERE `classname` = '%s' AND `steam` = 'STEAM_%i:%i:%i';", g_sTableName, g_sWeaponsClassName[i], g_iEngine == Engine_CSGO, g_iAccountID[iClient] & 1, g_iAccountID[iClient] >> 1);
		hTransaction.AddQuery(sQuery);
		g_iWeaponsStats[iClient][i] = 0;
	}

	g_hDatabase.Execute(hTransaction, _, SQLTransaction_FailureReset, _, DBPrio_High);
}

public void SQLTransaction_FailureReset(Database db, any data, int numQueries, const char[] sError, int failIndex, any[] queryData)
{
	LogError(PLUGIN_NAME ... " : ResetDataPlayer - %s", sError);
}