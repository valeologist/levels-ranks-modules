...
public void OnPluginStart()
{
	...
	{
		case Engine_CSGO, Engine_CSS: LoadTranslations("lr_module_skins.phrases");
...
void ConfigLoad() 
{
	...
	KeyValues hLR_Skins = new KeyValues("Skins");
	LR_GetTitleMenu(g_sPluginTitle, sizeof(g_sPluginTitle));

	if(!hLR_Skins.ImportFromFile(sPath))
	...
	do
	{
		hLR_Skins.GetSectionName(g_sSkinsName[g_iSkinsCount], sizeof(g_sSkinsName[]));
		hLR_Skins.GetString("skin", g_sSkinsModel[g_iSkinsCount], sizeof(g_sSkinsModel[]));
		PrecacheModel(g_sSkinsModel[g_iSkinsCount], true);
		g_iSkinsLevel[g_iSkinsCount] = hLR_Skins.GetNum("rank", 0);
		g_iSkinsCount++;
	}
