...
void ConfigLoad()
{
	...
	KeyValues hLR_Tags = new KeyValues("Tags");
...
{
	char sText[64];
	FormatEx(sText, sizeof(sText), "%T", !g_bOffTag[iClient] ? "TagRankOn" : "TagRankOff", iClient);
...
{
	char sCookie[2];
	GetClientCookie(iClient, g_hTagRank, sCookie, sizeof(sCookie));
	g_bOffTag[iClient] = sCookie[0] == '1';
}

public void OnClientDisconnect(int iClient)
{
	char sCookie[2];
	sCookie[0] = '0' + view_as<char>(g_bOffTag[iClient]);
	SetClientCookie(iClient, g_hTagRank, sCookie);
}
